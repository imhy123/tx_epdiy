...
```
main/
  epdiy_main.c          # Main logic: idf_setup() + minute clock in app_main()
                        # 主业务逻辑：idf_setup() + app_main() 里的分钟时钟
  epdiy_demo_main.c.bak # Backup of the original epdiy demo (NOT compiled)
                        # 原始 epdiy demo 备份（不参与编译）
  Kconfig.projbuild     # "Photo Frame Configuration" menu: e-paper display model
                        # menuconfig 配置菜单：墨水屏型号选择（默认 ES108FC2）
  idf_component.yml     # Component dependency: epdiy fork (see note below)
                        # 组件依赖：epdiy fork（见下方说明）
  font/firasans_*.h     # Generated font arrays (12/20/96px) — skip when reading code
                        # 工具生成的字体数组（12/20/96px），读代码时可跳过
  img/img_*.h           # Generated image arrays (used by the demo backup only)
                        # 工具生成的图片数组（仅 demo 备份用到）
partitions/
...
`E-paper display model`) and resolved by `selected_display()`. Default is
**ES108FC2** (this project's panel — note `bus_speed = 11`, not the generic 17).
当前面板在 menuconfig 里选，由 `selected_display()` 解析，默认 **ES108FC2**（本项目的屏，`bus_speed = 11`）。
```c
const EpdDisplay_t ES108FC2 = {
    .width = 1920, .height = 1080,
    .bus_width = 16, .bus_speed = 11,   // 11 MHz: 17 overruns this board (underrun)
    .default_waveform = &epdiy_ED047TC1,
};
#define TARGET_BOARD epd_board_v7  // ESP32-S3 uses v7 board / ESP32-S3 对应 v7 板
```
...
`epdiy_main.c` is a minute clock: `app_main()` calls `idf_setup()`, then loops —
each iteration draws `HH:MM` (FiraSans_96 on wide panels) into the framebuffer,
repaints just the clock area with `MODE_GL16` (a flashing `MODE_GC16` full refresh
on the hour), and sleeps until the next minute boundary. Time comes from the
system clock, which starts at 1970-01-01 00:00 on boot — real wall-clock time
still needs NTP (WiFi) or an RTC.

`epdiy_main.c` 现在是分钟时钟：`app_main()` 调 `idf_setup()` 后循环，每次把 `HH:MM`
（宽屏用 FiraSans_96）画进帧缓冲，只局部刷新时钟区域（整点做一次 GC16 全刷），
然后睡到下一分钟。时间来自系统时钟，开机从 1970-01-01 00:00 起算，真实时间还需 NTP/RTC。

Next step / 下一步: real time (WiFi + NTP or RTC), then calendar / photo / to-do
modules. / 接真实时间（WiFi+NTP 或 RTC），再做 日历/照片/待办 模块。
...
- `main/font/firasans_*.h` and `main/img/img_*.h` are large tool-generated arrays — skip them when reading code. New fonts are generated with epdiy's `scripts/fontconvert.py` (see README) / 是工具生成的大型数组，读代码时跳过；新字体用 epdiy 的 `scripts/fontconvert.py` 生成（见 README）
